function m1_indif = group_razomknut(L_mass_polovin,m1,NN)
%Функция распределения поля точек на группы
% Использован принцип ближайшего среднего расстояния 
%Распределяет поле на NN групп для решения разомкнутой задачи коммивояжера
%Используется на на нижних уровнях
%
% L_mass - Граф точек обсервации
% m1 - локалные точки обсервации

global inf 

%Предварительная работа
%Удаление из L_mass_polovin неучавствующх в расчете точек
for n = 1:length(L_mass_polovin)
    if n ~= m1
        L_mass_polovin(n,:) = inf;
        L_mass_polovin(:,n) = inf;
    end
end
...
while size(m1_indif,2) > NN  %Работает пока кол-во групп больше N
...
%if xx ~= yy %условие того, что точки не находятся в одной группе
